...
void test_decide_step_dir()
{
    ...
    assert(fabs(dda_a.side_dist_x - (ray_a.pos.x - dda_a.map_x) * dda_a.delta_dist_x) < EPS);
    assert(fabs(dda_a.side_dist_y - (dda_a.map_y + 1 - ray_a.pos.y) * dda_a.delta_dist_y) < EPS);
    ...
    assert(fabs(dda_b.side_dist_x - (dda_b.map_x + 1 - ray_b.pos.x) * dda_b.delta_dist_x) < EPS);
    assert(fabs(dda_b.side_dist_y - (ray_b.pos.y - dda_b.map_y) * dda_b.delta_dist_y) < EPS);
    
    printf("✓ decide_step_dir tests passed\n");
}

void test_step_until_hit()
{
...
void test_decide_draw_ends()
{
    ...
    int close_height = draw_end - draw_start;
    
    // Test far distance
    decide_draw_ends(&draw_start, &draw_end, screen_h, 10.0);
    int far_height = draw_end - draw_start;
...
void test_decide_wall_column()
{
    ...
    double expected_0 = game.player.pos.y + dda.perp_dist * ray.dir.y;
    assert(fabs(wall_x - expected_0) < EPS);
    
    // Test side == 1
    dda.side = 1;
    decide_wall_column(&wall_x, &dda, &ray, &game);
    double expected_1 = game.player.pos.x + dda.perp_dist * ray.dir.x;
